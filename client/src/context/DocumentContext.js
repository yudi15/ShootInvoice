...
export const DocumentProvider = ({ children }) => {
  ...
  const getDocument = useCallback(
    async (id) => {
      // Only fetch if we're not already loading and we don't already have this document
      if (!state.loading && (!state.document || state.document._id !== id)) {
        dispatch({ type: "SET_LOADING" });
        try {
  ...
  const createDocument = async (documentData) => {
    ...
    try {
      ...
      dispatch({
        type: "SET_DOCUMENTS",
        payload: [res.data, ...state.documents],
      });
      return res.data;
    } catch (err) {
      dispatch({
  ...
  const convertDocument = async (documentId, targetType) => {
    dispatch({ type: "SET_LOADING" });
    try {
      const res = await axios.post("/api/documents/convert", {
        documentId,
        targetType,
      });
      dispatch({
  ...
  const emailDocument = async (documentId, emailData) => {
    ...
    try {
      ...
      const response = await axios.post(
        `/api/documents/${documentId}/email`,
        {
